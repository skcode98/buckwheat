...
| Category widget | `app/.../widget/category/` (new 2026-08-15): `CategoryWidget.kt` (`CategoryWidgetDesign { BATTERY, COMPACT }`, pure `effectiveCategoryWidgetDesign(override, global)`, Responsive 220×110/160/240/400 → ≤2/3/5/all pills, `PILL_HEIGHT_DP=24f`, `CONTENT_WIDTH_DP=204f`, bitmap pill `drawCategoryPillBitmap` with `canvas.clipRect` fill — Glance 1.1.1 has no Canvas composables), `CategoryWidgetData.kt` (pure `categoryWidgetRows`/`categoryWidgetPills`/JSON `serialize`/`parse` tolerant, `CATEGORY_WIDGET_PALETTE_SIZE = 7`), `CategoryWidgetReceiver.kt` (`@AndroidEntryPoint : WidgetReceiver`), `CategoryWidgetConfigureActivity.kt` (per-instance override via `categoryDesignOverridePreferenceKey`, "Follow global" clears it) |
...
| History screen | `app/.../history/History.kt` — `composeHistoryRows` (now `internal`) groups `HistoryEntry`s into per-day cards (ascending then `reversed()` → newest day first), computes each card's `firstTransactionIndex` (running transaction count across ALL days ascending — feeds the tutorial) + `dayTotal` + a `contentHash` over every transaction in the day. Renders via `DayCard`. `onlyDay`/`onlyCategoryKey` filters + archived-rows-only-when-searching preserved |
| History list animation | `app/.../history/ListAnimation.kt` — `RowEntity(key, contentHash?, day, transactions, firstTransactionIndex, dayTotal?)` = ONE entry per DAY keyed `"day-$day"` (since 2026-08-15 redesign; used to be per-row `RowEntityType{DayDivider,Spent,DayTotal}`). `updateAnimatedItemsState` composites by key (reuse in place on content-hash match, fresh instance otherwise, removed days exit at their old spot), LazyColumn keyed by per-instance `AnimatedItem.id` — whole days animate in/out, a single edit updates the card in place. Tests: `app/src/test/.../history/ListAnimationTest.kt` (4, rewritten for the day-card model) |
| History day card | `app/.../history/DayCard.kt` (new 2026-08-15) — `RoundedCornerShape(22.dp)` `Surface` tinted `DayCardContainerColor` (`combineColors(surface, surfaceVariant, 0.3f)`), `DayCardHeader` (weekday + today/date left, `total_per_day` "Day total:" + amount right, `HorizontalDivider`), per-transaction `TimelineRail` + swipe `SwipeRowSheet` (matches card tint/corners). **PITFALL (crash fix 2026-08-15, Issue 34)**: do NOT wrap `SwipeActions` (a `BoxWithConstraints`/SubcomposeLayout) in `Modifier.height(IntrinsicSize.Min)` — the intrinsic-measure pass asks the SubcomposeLayout for `minIntrinsicHeight` and throws `IllegalStateException: Asking for intrinsic measurements of SubcomposeLayout layouts is not supported`. The transaction `Row` must NOT carry `IntrinsicSize.Min`; `IntrinsicSize.Min` is only safe around plain composables (e.g. inside `SwipeRowSheet`). |
| History timeline row | `app/.../history/TimelineRow.kt` (new 2026-08-15) — `categoryLabelFor` (built-in → `DEFAULT_EMOJI` + labelRes; custom → `SpendCategory.emojiFor` + stored name), `timelinePaletteFor` (category color via analytics `baseColors` → `harmonizeWithColor` → `toPalette`; OTHER neutral; custom hash to palette), `TimelineRail` (emoji dot + vertical connector), `TimelineRowContent` (comment/category label, "category · time" subtitle, right-aligned amount) |
| History row actions | `app/.../history/SpentItemActions.kt` — tap = edit, long-press = copy/edit/delete `DropdownMenu` (`buildSpendCopyText` pure helper); renders `TimelineRowContent`. `history/NoSpends.kt` empty state kept. Deleted 2026-08-15: `SpentItem.kt`, `HistoryDateDivider.kt`, `TotalPerDay.kt` |
| History grouping/hash tests | `app/src/test/java/.../history/HistoryRowsTest.kt` (6, new 2026-08-15) — grouping newest-first, filtering (`onlyDay`/`onlyCategoryKey`), day content-hash, archived-in-search |
