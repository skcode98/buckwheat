...
- **Voice widget chart (committed `8c4ead0`)**: drawn into a bitmap (Glance 1.1.1 has NO canvas/path composables — `androidx.glance.canvas` absent from the jars; `ContentScale` = `{ Fit, Crop, FillBounds }` only) via `drawChartBitmap(context, designWidthDp=280, chartHeightDp=26, ...)` in `VoiceWidget.kt`, rendered as `Image(provider=ImageProvider(bitmap), contentScale=ContentScale.FillBounds)`. Design palette: top `colorMax` (0xFFDD1414) → bottom `colorMin` (0xFF185ED6), night alphas 0.55/0.38/0.20, light 0.35/0.20/0.08; dashed goal line via `DashPathEffect(6dp,4dp)`; today marker = 8dp halo @0.2 + 4.5dp ring stroke 2dp + 3dp white dot on `fractions.last()`. On-device (API 36, 5×2): goal line ≈ y744, curve peak ≈ x402-555 y758-818, marker ≈ (1040,812); marker center = baseline.
