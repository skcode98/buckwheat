...
- **Spend entries can be dated before the period start / before today** — the spend editor's date picker no longer blocks dates earlier than the current period start (`DateTimeEditPill` dropped the `disableBeforeDate = startPeriodDate` floor) and past months are reachable again (the base calendar now defaults to a one-year-back range when no floor is set). Entries dated outside the period are recorded but excluded from the current period's budget arithmetic, matching the existing `addSpent` out-of-period guard
...
- No notification system, no sync, no reminder (upstream-only features; see Future Considerations in `MEMORY.md`)
