...
| Putting a `SubcomposeLayout`-based composable (`BoxWithConstraints`, `AnimatedVisibility`, lazy content) under `IntrinsicSize.Min`/`IntrinsicSize.Max` | Compose runs an intrinsic-measure pass and asks the SubcomposeLayout for intrinsic sizes it can't compute → `IllegalStateException: Asking for intrinsic measurements of SubcomposeLayout layouts is not supported`. E.g. `DayCard.kt` once wrapped `SwipeActions` (a `BoxWithConstraints`) in `Row(Modifier.height(IntrinsicSize.Min))` — History crashed on swipe. `IntrinsicSize.*` is only safe around plain leaf composables; add an explicit size modifier or drop the intrinsic wrapper (LESSONS.md Issue 34) |
