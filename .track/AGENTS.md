...
- **Backup branch**: `backup/pre-trackinvest-2026-08-09` on origin — frozen snapshot of `master` @ `bc5e922` (all achievements up to 2026-08-09) taken before the TrackInvest web-app → Android migration started. Do NOT delete; it is the pre-migration rollback point.
- **Workflow**: Build APK via GitHub Actions (`.github/workflows/build.yml`)
- **Pushed so far (2026-08-09)**: `96feeec` (AI-voice root cause: default model → `openai/gpt-oss-20b:free`, defaults centralized in `SettingsRepository.kt` with `normalizeVoiceAiModel` legacy-upgrade, all 5 consumers updated — VoiceAi, settings sheet, settings VM, SpendCategorizer; **also** daily budget reminder ported from `setRepeating` to `setWindow` + receiver re-arm), `dbee831` (AI-voice diagnosis step 1 — widget voice path surfaces the AI failure reason instead of silently committing offline results: `ParsedTranscript(results, aiFailure)` + `voice_widget_ai_off` "AI off: <reason>" in notification/caption + `Log.w("VoiceAI", …)` on widget & in-app paths), `81b7ab2` (daily midnight widget refresh — `WidgetRefreshScheduler` `setWindow` 00:05 10-min window + `WidgetRefreshReceiver`, re-arm on fire, boot reschedule), `855e240` (analytics "vs previous" fix — `effectiveFinishDate = actualFinishDate ?: finishDate`, day-granularity compare in `findPreviousPeriod`/`previousPeriodBefore`), `cedc0e0` (voice widget 16dp corners via explicit first-child Image instead of `background(ImageProvider)` + app-pause refresh), `9432172` (voice widget design switcher — Settings row + dialog, Percent left / Amount left / Ring layouts), `75e12c2` (voice widget big-percent design: % of daily budget remaining as the hero + thin budget progress bar + "of ₹X daily" line), `492d7c8` (voice widget redesign: daily remaining as the big number + min/max chart markers + budget/spent/left summary row), `8c4ead0` (voice widget min/max-style spend chart — bitmap-drawn cubic-bezier area + dashed goal line + today marker), `e8162a2` (voice widget: add another spend after a commit — tappable green check + 4s auto-reset), `8c2ba4f` (voice widget spend chart + live input feedback), `e2370e3` (widget POST_NOTIFICATIONS on the mic tap path), `abe131c`/`7d32713`/`9c2f0ee` (voice widget render fixes), `8a6c55d` (voice AI free-model dropdown), `cb6f72f` (on-track overspend alert), `8dce722`/`18926d5` (review-wave fixes), `1fa1534` (voice input app widget), `a147c2d` (weekday spend breakdown), `3bbdd9c` (instant overspend notification), `e7249c9` (share spending summary + compare-to-last-period card), `9a75b05` (full JSON backup & restore), `d389a0e` (emoji picker for custom categories). Earlier: `f6e3649`, `2c90b8d`, `72db09f`, `3a83f7c`, `fc16848`, `328dd94`, `212f7cf`, `a309c29` (see CHANGELOG)
- **Test baseline**: `.\gradlew.bat :app:testDebugUnitTest` green (**153 tests**, 14 suites): 40 `VoiceInputParserTest`, 26 `SpendsRepositoryTest`, 18 `SpendCategorizerTest`, 10 `SpendsTrendTest`, 11 `CompareToLastPeriodTest` (7 of them the 2026-08-08 "vs previous" regression set), 8 `BackupDataTest`, 7 `SpendsWeekdayTest`, 4 `DailyReminderContentTest`, 4 `ListAnimationTest`, 4 `ShareSummaryTest`, 4 `BackupRepositoryTest`, plus the voice widget tests (`VoiceWidgetCommitTest`, `VoiceWidgetRenderTest` — Robolectric SDK 35). NOTE: Robolectric `widget.update()` never runs `provideGlance`, so widget render tests are compile-level only; on-device emulator E2E is authoritative.
...
│
├── history/
│   ├── History.kt              # Transaction history list — flat per-day list (DayDivider + Spent rows + TotalPerDay; the 2026-08-15 day-card redesign was REVERTED 2026-08-16, commit b581ab1)
│   ├── SpentItem.kt            # Spend item row (amount + date, category pill, comment; bottom padding 18dp) [restored 2026-08-16]
│   ├── HistoryDateDivider.kt   # Day divider header [restored 2026-08-16]
│   ├── TotalPerDay.kt          # "Day total" row [restored 2026-08-16]
│   ├── Spent.kt                # Spend item row (composable primitive)
│   ├── SpentItemActions.kt     # Tap-edit + long-press copy/edit/delete menu (renders SpentItem)
│   ├── SwipeActions.kt         # Swipe-to-edit/delete (LazyColumn item root — never wrap in IntrinsicSize.*)
│   ├── NoSpends.kt
│   └── ListAnimation.kt        # RowEntityType { DayDivider, Spent, DayTotal } per-row animation (day-card model removed 2026-08-16)
│
├── analytics/
...
│   ├── CanvasText.kt
│   ├── category/               # "Buckwheat categories" utilization widget (CategoryWidget, CategoryWidgetData, CategoryWidgetReceiver, CategoryWidgetConfigureActivity)
│   ├── minimal/
│   └── extend/
│
...
| Putting a `SubcomposeLayout`-based composable (`BoxWithConstraints`, `AnimatedVisibility`, lazy content) under `IntrinsicSize.Min`/`IntrinsicSize.Max` | Compose runs an intrinsic-measure pass and asks the SubcomposeLayout for intrinsic sizes it can't compute → `IllegalStateException: Asking for intrinsic measurements of SubcomposeLayout layouts is not supported`. E.g. the 2026-08-15 `DayCard.kt` redesign once wrapped `SwipeActions` (a `BoxWithConstraints`) in `Row(Modifier.height(IntrinsicSize.Min))` — History crashed on swipe (the redesign was reverted 2026-08-16, so `SwipeActions` is again the direct LazyColumn item root). `IntrinsicSize.*` is only safe around plain leaf composables; add an explicit size modifier or drop the intrinsic wrapper (LESSONS.md Issue 34) |
| Glance widget compile/asset assumptions (battery-pill bitmaps, text, preview drawable) | Glance 1.1.1 has NO Canvas/path composables — draw pills into bitmaps (`canvas.clipRect`) rendered as `Image`. `TextStyle.color` needs `.toColorProvider()`; `TextUtils.ellipsize` needs an `android.text.TextPaint`. Verify every `@color/...` referenced by a drawable/vector actually exists in `colors.xml`. Never edit Kotlin with PowerShell `-replace` (case-insensitive mangles identifiers) (LESSONS.md Issue 33) |
| Glance `ColorProvider` reflection / widget-level failure | `ColorProvider` is an **interface** in Glance 1.1.1 — `getDeclaredConstructor` always throws; use the top-level `ColorProvider(color)` factory (`widget/CommonWidgetReceiver.kt` `toColorProvider`). A `catch (e: Exception)` does NOT catch `Error` (the `LocalContentColor.current` default `{ throw Error("No set") }` escaped it → widget "Can't show content"). A failure isolated to ONE widget usually means something THAT widget doesn't provide that the others do (LESSONS.md Issue 35) |
| Editor date picker bounds | The picker's ENABLED range (`disableBeforeDate`/`disableAfterDate`) and SHOWN months (`showBeforeDate`/`showAfterDate` in `CalendarState`, default = disable bounds) are independent. Editor passes start→today enable, start→finish show. Two paths must stay in sync or dates are "rendered but unselectable" / "selectable but invisible". Re-check the whole chain (`CalendarState` → `CalendarUiState.isDisabledDay` → rendered months) when a date-picker bug is re-reported (LESSONS.md Issue 28 + amendment) |
