...
fun OnTrackAlertSetting() {
    ...
    LaunchedEffect(Unit) {
        ...
        enabled = prefs[onTrackAlertEnabledStoreKey] ?: false
        hour = prefs[onTrackAlertHourStoreKey] ?: DAILY_REMINDER_DEFAULT_HOUR
        minute = prefs[onTrackAlertMinuteStoreKey] ?: DAILY_REMINDER_DEFAULT_MINUTE
    }

    fun enableAlert() {
        enabled = true
        coroutineScope.launch {
            context.settingsDataStore.edit { it[onTrackAlertEnabledStoreKey] = true }
        }
    ...
    fun onToggle() {
        if (enabled) {
            ...
            coroutineScope.launch {
                context.settingsDataStore.edit { it[onTrackAlertEnabledStoreKey] = false }
            }
        } else {
    ...
    Row(
        ...
        Text(
            text = stringResource(R.string.on_track_alert_setting_title),
    ...
    if (enabled) {
        TextRow(
            ...
            iconTint = iconTint,
            text = stringResource(R.string.on_track_alert_time),
            ...
            onSelect = { selectedHour, selectedMinute, _ ->
                ...
                coroutineScope.launch {
                    context.settingsDataStore.edit {
                        it[onTrackAlertHourStoreKey] = selectedHour
                        it[onTrackAlertMinuteStoreKey] = selectedMinute
                    }
                }
                OnTrackAlertScheduler.schedule(context, selectedHour, selectedMinute)
            },
